	...
	"Person Services" should {
    	...
    	"be able to create a person with one email address" in {
    		...
    		val expectedPerson = Person(None,
		      ...
		      actualPerson.contactData.emailAddresses.get.head.address must be_==("someone@somewhere.com")
		      actualPerson.contactData.emailAddresses.get.head.location must be_==(Home)
    	}
	    ...
	    "be able to create a person with one phone number" in {
		    ...
		    actualPerson.contactData.phoneNumbers.get.head.number must be_==("555-555-12345")
		    actualPerson.contactData.phoneNumbers.get.head.location must be_==(scala_mash.highrise_api.models.PhoneLocationValues.Home)
	    }
	 	...
	 	"be able to create a person with one address" in {
		    ...
		    actualPerson.contactData.addresses.get.head.city must be_==("phoenix")
		    actualPerson.contactData.addresses.get.head.state must be_==("az")
		    actualPerson.contactData.addresses.get.head.street must be_==("111 N somewhere")
		    actualPerson.contactData.addresses.get.head.zip must be_==("12345")
		    actualPerson.contactData.addresses.get.head.location must be_==(scala_mash.highrise_api.models.AddressLocationValues.Home)
		}
		...
		"be able to create a person with one instant messenger" in {
		    ...
		    actualPerson.contactData.instantMessengers.get.head.address must be_==("username")
		    actualPerson.contactData.instantMessengers.get.head.protocol must be_==(InstantMessengerProtocolValues.Other)
		    actualPerson.contactData.instantMessengers.get.head.location must be_==(InstantMessengerLocationValues.Other)
		}
		...
		"be able to create a person with one web address" in {
		    ...
		    actualPerson.contactData.webAddresses.get.head.url must be_==("http://somewhere.com")
		    actualPerson.contactData.webAddresses.get.head.location must be_==(InstantMessengerLocationValues.Other)
	  	}
		...
		"be able to find a single person by email" in {
	    	...
	    	val actualPerson: Person = Person.create(expectedPerson, account)
			val found = Person.findPeopleByEmail(expectedPerson.contactData.emailAddresses.get.head.address, account)
		    actualPerson.id must beSome[Long].which(_ > 0)
		    actualPerson.createdAt must beSome[DateTime]
		    actualPerson.updatedAt must beSome[DateTime]
		    actualPerson.firstName must be_==(expectedPerson.firstName)
		...
		"A LoginFailed exception should be thrown when a bad highrise sitename is used to find by email" in {
	    	...
	    	val actualPerson: Person = Person.create(expectedPerson, account)
				...
				Person.findPeopleByEmail(expectedPerson.contactData.emailAddresses.get.head.address, badAccount) must throwA[LoginFailed]
		}
